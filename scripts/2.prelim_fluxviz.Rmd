...
## methane flux by individual mounds, comparing seasons, by species
```{r flux}
# all mounds
...
## mound and soil methane flux by factor(position)
```{r by position}
# mounds 
c1 <- ggplot(data = mounds_ID, aes(x = as.factor(position), y = flux.CH4, fill = ID_cleaned)) + 
  geom_boxplot()+ 
  facet_wrap(~campaign, scales = "free_x")+
  ylab("CH4 flux umol/d/m2")+
  theme_light()
c2 <- ggplot(data = mounds_ID, aes(x = as.factor(position), y = ch4_co2, fill = ID_cleaned)) + 
  geom_boxplot()+
  facet_wrap(~campaign, scales = "free_x")+
  ylim(0,50)+ # for outliers
  theme_light()
...
# scatter plot - methane 
# average temperature by mound via campaigh
