...
relative abundance for pathway table (non-standardised v standardised)
```{r}
...
e <- ggplot(each_pathway_relabund, aes(x = pathway, y = mean_relabund, fill = pathway)) +
  geom_bar(stat = "identity") +
  labs(title = "Mean realative abundance per pathway", x = "Pathway", y = "Relative abundance") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "none")

f <- ggplot(each_pathway_relabund, aes(x = pathway, y = mean_resampled_relabund, fill = pathway)) +
  geom_bar(stat = "identity") +
  labs(title = "Mean realative abundance per pathway", x = "Pathway", y = "Relative abundance") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "none")
...
merge metadata with methane metabolism pathway relative abundances 
```{r}
flux_relabund <- left_join(methane_pathway, metadata, by = c("sequencing_id"))

ggplot(flux_relabund, aes(x = resampled_relabund, y = flux.CH4)) + 
  geom_point() + 
...
# TODO add wall thickness and temperature to the model
...
# TODO 
# normalise for the number of reads by the size of the library (total # reads, gene length) 
```
