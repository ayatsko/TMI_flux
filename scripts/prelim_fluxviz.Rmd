...
```{r}
avg <- all_fluxes_resample %>%
  group_by(species_s, campaign) %>%
  summarise(avg_CH4 = mean(flux.CH4, na.rm = TRUE), 
            avg_CO2 = mean(flux.CO2, na.rm = TRUE))
```
...
s1 <- ggplot(data = all_fluxes_resample[all_fluxes_resample$flux_source =="m",], aes(x = species_s, y = flux.CH4, fill = campaign)) + 
  geom_boxplot()+ 
...
s2 <- ggplot(data = all_fluxes_resample[all_fluxes_resample$flux_source =="m",], aes(x = species_s, y = flux.CO2, fill = campaign)) + 
  geom_boxplot()+ 
  scale_x_discrete(limits = c("N. magnus", "A. laurensis", "C. acinaciformis"))+  
  theme_classic()+
