...
Draft models for predictor of daily variation in fluxes 
```{r}
# testing for intramound variation - does position affect flux? 
# create unique ID for mounds in different sampling periods throughout the day
CH4_day$ID_mound <- paste(CH4_day$sample, CH4_day$time_of_day, sep = "-")

# check CoV
detach(package:plyr)
cov <- CH4_day %>% group_by(ID_mound) %>% 
  summarize(cov = sd(flux.CH4) / mean(flux.CH4))
ggplot(cov, aes(cov)) +
  geom_histogram(aes(y=after_stat(density)), colour="black", fill="white")+
 geom_density(alpha=.2, fill="#FF6666") +
  theme_classic()
x <- cov[cov$cov > 1, ] # 15/36 mounds have high CoV - test these further
names <- x$ID_mound
test <- CH4_day[CH4_day$ID_mound %in% names,]
