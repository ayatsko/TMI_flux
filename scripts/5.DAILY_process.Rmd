...
UNDER CONSTRUCTION remove/fix QC flagged fluxes 
```{r}
...
# read in troubleshooting metadata
...
x <- trouble_test[1,]
x_new <- x[ , colSums(is.na(x))==0]

if("new_start" %in% colnames(x_new)){
  new_start_val <- x_new$new_start;
}

if("new_end" %in% colnames(x_new)){
  new_end_val <- x_new$new_end;
}

testy <- data_merge_day[!data_merge_day$fDOY < new_start_val, ]
testy <- data_merge_day[!data_merge_day$fDOY > new_end_val, ]

ggplot(testy[testy$ID_measurement == problem[1],], aes(x = fDOY, y = CO2_dry))+
  geom_point()+
  theme_classic()
```
