...
```{r}
setwd("/Users/abbeyyatsko/Desktop/repos/TMI_flux/data/LGR_files/aug23LGR_daily")
...
Merge with metadata
```{r}
meta_TMI_day <- read.csv('/Users/abbeyyatsko/Desktop/repos/TMI_flux/data/LGR_metadata/TMI_flux_metadata_aug_daily.csv')
...
species <- read.csv("/Users/abbeyyatsko/Desktop/repos/TMI_flux/data/supp_data/daily_species.csv")
...
# write.csv(CO2_fluxfinal_aug23_day,"/Users/abbeyyatsko/Desktop/repos/TMI_flux/data/data_out/CO2_fluxfinal_aug23_day.csv", row.names = FALSE)

# CH4 (units are umol/day)
# write.csv(CH4_fluxfinal_aug23_day,"/Users/abbeyyatsko/Desktop/repos/TMI_flux/data/data_out/CH4_fluxfinal_aug23_day.csv", row.names = FALSE)
```
...
CO2_fluxfinal_aug23_day_fixed$ID_measurement <- paste(CO2_fluxfinal_aug23_day_fixed$sample, CO2_fluxfinal_aug23_day_fixed$tag, CO2_fluxfinal_aug23_day_fixed$time_of_day, sep = "-")
CH4_fluxfinal_aug23_day_fixed$ID_measurement <- paste(CH4_fluxfinal_aug23_day_fixed$sample, CH4_fluxfinal_aug23_day_fixed$tag, CH4_fluxfinal_aug23_day_fixed$time_of_day, sep = "-")

# pull out samples that have p < 0.05
table(CO2_fluxfinal_aug23_day_fixed$p.CO2<0.05) # all CO2 is fine (either removed or fluxes are fixed)
table(CH4_fluxfinal_aug23_day_fixed$p.CH4<0.05)
a <- CH4_fluxfinal_aug23_day_fixed[CH4_fluxfinal_aug23_day_fixed$p.CH4>0.05,]
...
zero_flux <- function(CH4_fluxfinal_aug23_day_fixed, CH4_to_zero, assign_zeros_CH4) {
  # Replace columns in assign_zeros_CH4 with 0 for ID_measurement values in CH4_to_zero
  CH4_fluxfinal_aug23_day_fixed <- CH4_fluxfinal_aug23_day_fixed %>%
    mutate_at(vars(all_of(assign_zeros_CH4)), ~ ifelse(ID_measurement %in% CH4_to_zero, 0, .))
  
  return(CH4_fluxfinal_aug23_day_fixed)
}

CH4_fluxfinal_aug23_day_fixed_zeroadj <- zero_flux(CH4_fluxfinal_aug23_day_fixed, CH4_to_zero, assign_zeros_CH4)
```
...
# CH4 (units are umol/day)
# write.csv(CH4_fluxfinal_aug23_day_fixed_zeroadj,"/Users/abbeyyatsko/Desktop/repos/TMI_flux/data/finalfluxes/CH4_fluxfinal_aug23_day_fixed.csv", row.names = FALSE)
```

calculate Q10
```{r}
# first look - isolate a single mound 
DM <- CH4_day[CH4_day$sample == "DM1",]
plot(DM$avg_respT, DM$flux.CH4)

# look at range of temperatures 
min(DM$avg_respT); max(DM$avg_respT)

test <- DM[order(DM$avg_respT),]
t <- test$avg_resp
r <- test$flux.CH4
Q10(R_vec = r, T_vec = t)

# get list of mound IDs 
x <- unique(CH4_day$sample)

# Q10 associated with each individual mound
for (i in x){
  df <- CH4_day[CH4_day$sample == i,]
  t <- df$avg_resp
  r <- df$flux.CH4
  print(Q10(R_vec = r, T_vec = t))
}
...
for(i in x){
  df <- CH4_day[CH4_day$sample == i,]
  plot(df$avg_respT, df$flux.CH4, main = paste("Mound", i),)
}

# TODO mound as a random effect in Q10 model, also try model with mound averages
# same thing with CO2 
```
