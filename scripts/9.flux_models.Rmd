...
summary(emms)
emtr <- emtrends(mod, ~ species_s, var = "wall_thickness_centered")
summary(emtr)
...
# mean mean_mound_fluxCH4 by campaign table
mean_mound_fluxCH4_by_campaign <- all_fluxes_resample_reps_avg %>%
  group_by(campaign) %>%
  summarise(mean_mound_fluxCH4 = mean(mean_mound_fluxCH4, na.rm = TRUE))
