...
CH4_summary <- CH4_fluxfinal[, c("chunk","species", "ID", "material", "mass_termite_g", "flux.CH4", "sd.CH4", "R2.CH4", "p.CH4", "Time_start", "fDOY_end")]
...
JUST MESSIN AROUND, MAN (aka open questions)
```{r}
# how much methane is oxidized
meth_ox <- df_filtered %>%
  group_by(chunk) %>%
  mutate(all = ifelse(material == "A", flux.CH4, NA), 
         termite = ifelse(material == "T", flux.CH4, NA)) 

out <- meth_ox[c("chunk", "species", "all", "termite")]

compiled_meth_ox <- out %>%
  group_by(chunk) %>%
  summarize_all(funs(trimws(paste(unique(na.omit(.)), collapse = ", ")))) %>%
  ungroup()

compiled_meth_ox$all <- as.numeric(compiled_meth_ox$all)
compiled_meth_ox$termite <- as.numeric(compiled_meth_ox$termite)

compiled_meth_ox$prop_ox <- (compiled_meth_ox$termite - compiled_meth_ox$all) / compiled_meth_ox$termite
ggplot(compiled_meth_ox, aes(x = species, y = prop_ox, fill = species))+
  geom_boxplot()+
  geom_jitter()+
  theme_classic()+
  xlab("")+
...
# test if these values are significantly different from zero? 

# species means 
detach(package:plyr)
species_mean_ox <- compiled_meth_ox %>%
  group_by(species) %>%
  summarize(mean_ox = mean(prop_ox, na.rm = TRUE))

# co2 proportions 
co2_prop <- df_filtered %>%
  group_by(chunk) %>%
  mutate(all = ifelse(material == "A", flux.CO2, NA), 
         termite = ifelse(material == "T", flux.CO2, NA)) 

out <- co2_prop[c("chunk", "species", "all", "termite")]

compiled_co2_prop <- out %>%
  group_by(chunk) %>%
  summarize_all(funs(trimws(paste(unique(na.omit(.)), collapse = ", ")))) %>%
  ungroup()

compiled_co2_prop$all <- as.numeric(compiled_co2_prop$all)
compiled_co2_prop$termite <- as.numeric(compiled_co2_prop$termite)
compiled_co2_prop$mound <- compiled_co2_prop$all - compiled_co2_prop$termite

compiled_co2_prop_long <- pivot_longer(compiled_co2_prop, cols = 3:5 , names_to = "source", values_to = "CO2")

ggplot(compiled_co2_prop_long, aes(x = species, y = CO2, fill = source))+
  geom_boxplot()+
  geom_jitter()+
  theme_classic()+
  xlab("")

# looks like amitermes have little microbial production of co2, coptotermes has a lot of microbial respiration, and nasutitermes may have some kind of photosynthesis going on in the mound? 
```
